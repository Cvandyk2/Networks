...
edges <- read.csv("~/Desktop/GraduateSchool/S4/Network/FinalProj/GovEges.csv")
...
+ nodes
```{r}
net2 <- igraph::add_vertices(net, nv = 4) #add 4 vertices
net2
ggraph(net2) +
  geom_edge_link(color = "black", alpha = 0.3) +
  geom_node_point(color = "black", fill = "#efd9d3", shape = 21) +
...
+ edges
```{r}
net3 <- igraph::add_edges(net2, c(sample(V(net2), 4),sample(V(net2), 4))) #add edges, randomly
net3
...
- edges
```{r}
net4 <- igraph::delete_edges(net3, c(sample(E(net3), 3))) #delete 3 edges from the list at random
net4
...
- nodes
```{r}
net5 <- igraph::delete_vertices(net4, c(sample(length(nodes$ego_id), 3))) #delete nodes, randomly
...
```{r}
net6 <- igraph::rewire(net5, each_edge(p = .15, loops = F)) #15% chance of each edge being rewired
net6
ggraph(net6) +
  geom_edge_link(color = "black", alpha = 0.3) +
  geom_node_point(color = "black", fill = "#efd9d3", shape = 21) +
...
plot(sm501, comp = "NR", xlim = c(0, 2), ylim = c(0, 11)) #plotting all simulations at once, for recovered
```
...
plot(sm50, comp = "NR", xlim = c(0, 5), ylim = c(0, 11)) #plotting all simulations at once, for recovered
```
